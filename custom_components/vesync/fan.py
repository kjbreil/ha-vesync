...
def _setup_entities(devices, async_add_entities):
    ...
    for dev in devices:
        ...
        elif isinstance(dev, VeSyncHumid200300S):
            entities.append(VeSyncHumidiferMistHA(dev))
            if "warm_mist" in dev.features:
                entities.append(VeSyncHumidiferWarmMistHA(dev))
        else:
            _LOGGER.warning(
...
class VeSyncHumidiferMistHA(VeSyncDevice, FanEntity):
    ...
    def __init__(self, fan):
        """Initialize the VeSync fan device."""
        super().__init__(fan)
        self._attr_unique_id = f"{super().unique_id}-mist"
        self._attr_name = f"{super().name} mist"

    @property
    def speed_count(self) -> int:
        """Return the number of speeds the fan supports."""
        return len(self.device.mist_levels) - 1

    @property
    def percentage(self) -> int | None:
        """Return the current speed."""
        return ranged_value_to_percentage(
            (1, len(self.device.mist_levels) - 1),
            self.device.details["mist_virtual_level"],
        )
...
class VeSyncHumidiferWarmMistHA(VeSyncDevice, FanEntity):
    """Representation of a VeSync Humidifier Mist Speed as Fan."""
    ...
    def __init__(self, fan):
        """Initialize the VeSync fan device."""
        super().__init__(fan)
        self._attr_unique_id = f"{super().unique_id}-warm_mist"
        self._attr_name = f"{super().name} warm mist"

    @property
    def speed_count(self) -> int:
        """Return the number of speeds the fan supports."""
        return len(self.device.warm_mist_levels) - 1

    @property
    def percentage(self) -> int | None:
        """Return the current speed."""
        return ranged_value_to_percentage(
            (1, len(self.device.warm_mist_levels) - 1),
            self.device.details["warm_mist_level"],
        )

    def set_percentage(self, percentage: int) -> None:
        """Set the speed of the device."""
        self.device.set_warm_level(
            math.ceil(
                percentage_to_ranged_value(
                    (1, len(self.device.warm_mist_levels) - 1),
                    percentage,
