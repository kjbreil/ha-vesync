...
from .common import VeSyncDevice, VeSyncBaseEntity
...
async def async_setup_entry(
    ...
    config_entry.async_on_unload(
        async_dispatcher_connect(
            hass, VS_DISCOVERY.format(VS_SWITCHES), discover
        )
    )
...
def _setup_entities(devices, async_add_entities):
    ...
    for dev in devices:
        if DEV_TYPE_TO_HA.get(dev.device_type) == "outlet":
            ...
            entities.append(VeSyncDisplaySwitch(dev))
        else:
            _LOGGER.warning(
...
class VeSyncDisplaySwitch(VeSyncBaseSwitch, SwitchEntity):
    """Handle representation of VeSync Light Switch."""
    ...
    def __init__(self, switch):
        ...
        self._attr_name = f"{super().name} display"
