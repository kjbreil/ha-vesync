    ...
    async def async_new_device_discovery(service: ServiceCall) -> None:
        ...
        if new_switches and switches:
            ...
            async_dispatcher_send(
                hass, VS_DISCOVERY.format(VS_SWITCHES), new_switches
            )
            return
        if new_switches and not switches:
            switches.extend(new_switches)
            hass.async_create_task(
                forward_setup(config_entry, Platform.SWITCH)
            )
        ...
        if new_lights and lights:
            ...
            async_dispatcher_send(
                hass, VS_DISCOVERY.format(VS_LIGHTS), new_lights
            )
            return
        ...
        if new_sensors and sensors:
            ...
            async_dispatcher_send(
                hass, VS_DISCOVERY.format(VS_SENSORS), new_sensors
            )
            return
        if new_sensors and not sensors:
            sensors.extend(new_sensors)
            hass.async_create_task(
                forward_setup(config_entry, Platform.SENSOR)
            )
